...
> Last updated: May 27, 2026 | Maintained by: Co-founders
...
Robin is a trades-specific credential wallet, knowledge community, and earned reputation platform designed to solve the Canadian skilled trades workforce shortage by providing:

1. **Portable credential wallet** — Open Badge credentials owned by the individual, not the employer or union. Red Seal, safety tickets, apprentice certs — all stored, verifiable, and portable across provinces and employers.
2. **Earned reputation** — Expert status is earned through demonstrated community knowledge, not self-declared or gifted by coworkers clicking a button.
...
5. **Skill mapping & continuous learning** — Compare your expertise against National Occupational Standards (NOS), identify gaps, and surface relevant training resources to close them

**Robin is not LinkedIn for trades.** Reputation on Robin is earned, not given. A journeyperson with 850 points in panel installation got there because 47 of their peers found their answers credible enough to upvote — not because their coworkers clicked a button. Credibility is **earned, demonstrated, verified, and weighted**.
...
- [ ] **Accepted answer** — question author can mark one answer as accepted; accepted answer is pinned to top with visual checkmark; answerer receives 15 reputation points
- [ ] **Community promotion fallback** — if no answer is accepted and an answer exceeds the upvote threshold, it is auto-promoted to top position (protects answerers when question asker never returns)
- [ ] Content detail view (full thread with answers, accepted answer pinned, peer endorsements)
- [ ] Topic / tag browse (aligned to NOS block taxonomy)
- [ ] Full-text search (Algolia-powered)
- [ ] Saved content / bookmarks (personal, offline accessible)
- [ ] Content reporting (flag unsafe, off-topic, or harmful content)
...
- One accepted answer per question (accepting a new one un-accepts the previous)
- Cannot accept your own answer
- Community promotion triggers when upvote count ≥ 10 with no accepted answer after 14 days

**Success metric:** 200+ published posts per week by end of Phase 1; 30%+ of resolved questions have an accepted answer.
...
- [ ] **Public user profiles** — all profiles are public (see Section 10.3)
- [ ] **Expertise topic scores** — separate from overall reputation; per-topic scores visible on profile
- [ ] **Leaderboards** — ranked by expertise topic; visible to all; drives Loop 2 engagement

**Reputation score transparency — scoring weights are public to all users:**

| **Event** | **Points** |
|---|---|
| Accepted answer | 15 pts |
| Community-promoted answer | 8 pts |
| Upvote received | 5 pts |
| Peer endorsement (verified, same trade) | Weighted by endorser tier × 1.0–3.0× |
| Audit pass | 25 pts |
| Audit fail | -15 pts |

**Two separate score types (not interchangeable):**
- **Overall reputation score** — your platform standing; drives tier (apprentice/journeyperson/master)
- **Expertise topic scores** — your subject-matter credibility in specific NOS-mapped topics; drives leaderboards and skill mapping

Two tradespeople can have identical overall scores with completely different expertise profiles. This is intentional — it makes the data meaningful for recruiters, training companies, and the tradespeople themselves.

**Success metric:** 3+ endorsements per 10 active users; <1% fraudulent credentials flagged in audit.

---

### **Phase 2: Skill Mapping, Mentorship & Institutional Features**

**Scope:** Skill mapping & gap analysis, earned mentor tier (Zone 3), mentorship matching (Zone 3), credential issuance (Zone 3), basic institutional admin (Zone 5), training resource catalogue.

#### **Skill Mapping & Gap Analysis (Phase 2)**

Robin surfaces skill gaps by comparing a user's expertise topic scores against National Occupational Standards (NOS) targets for their role tier.

**Critical design principle:** Robin mirrors standards, never defines them. NOS competency blocks are published by ESDC Canada and used industry-wide. Robin measures community activity against them — Robin is not a certifier, regulator, or competency assessor.

- [ ] NOS block score view — per-user rollup of topic scores to NOS block level (e.g., Block B: Residential Wiring — 425 pts)
- [ ] Skill gap identification — gaps surfaced when block score is below NOS role target
- [ ] Training resource recommendations — curated courses and programs mapped to NOS blocks
- [ ] Gap dismiss — user can dismiss a recommendation with a note
- [ ] Gap resolution — auto-resolved when score reaches target

**Training resource catalogue:**
- Phase 2: Robin-curated free listings (NAIT, SAIT, BCIT, IBEW, exam prep providers mapped to NOS blocks)
- Phase 3: Paid provider placement (training companies pay for priority surfacing — clearly labelled, never disguised as organic)

**Legal safeguard:** All skill gap displays carry a disclaimer: Robin scores reflect community activity against NOS competency frameworks published by ESDC. Robin does not certify competency and scores are not a substitute for official certification, licensing, or employer assessment.

#### **Mentorship (Phase 2)**

**Note:** Formal mentorship is intentionally deferred from Phase 1. Q&A activity builds the data and organic relationships first. In Phase 2, mentorship is surfaced to users who are already consistently helping the same people — a warmer and more authentic entry point than cold requests to strangers.

- [ ] Mentor tier unlocked at 750 reputation points (sustained contribution over months — cannot be gamed quickly)
- [ ] Browse available mentors (filtered by trade, specialisation, availability)
...
- [ ] Employer dashboard (workforce credential visibility, skill gap analysis by employee)
- [ ] Institutional bulk verification upload (feature-flagged off in Phase 1 — enable when first union requests it)

**Success metric:** 50+ active mentorships; 10+ users reaching mentor tier; skill gap feature used by 30%+ of active users.
...
### **10.3 Public User Profiles (Phase 1)**

**Decision:** All Robin profiles are public. There is no private profile setting.

**Rationale:** The entire value of Robin is built on trust and signal — your reputation, credentials, and endorsements. A private profile user consumes community knowledge without contributing to the social fabric that makes the platform credible. Participation is the contract.

**Profile visibility rules:**
- All profiles are public after a 30-day onboarding grace period
- During the grace period, profiles are accessible by direct URL but excluded from search results and leaderboards
- After 30 days, profiles automatically become fully public (background job)
- There is no permanent private profile option — if this is a dealbreaker for a user, Robin is not the right platform for them

**What is public on a profile:**

| **Field** | **Visible** | **Notes** |
|---|---|---|
| Display name | ✅ Public | |
| Avatar | ✅ Public | |
| Role (apprentice/journeyperson/master) | ✅ Public | |
| Province | ✅ Public | |
| Years of experience | ✅ Public | |
| Bio | ✅ Public | Optional — user controls content |
| Trades | ✅ Public | |
| Reputation tier & score | ✅ Public | Core trust signal |
| Expertise topic scores | ✅ Public | Top topics visible on profile |
| Credentials (professional wallet) | ✅ Public | With verification watermark |
| Platform-earned credentials | ✅ Public | Contribution badges, endorsements |
| Recent activity (top 5) | ✅ Public | Last 5 published answers/posts |
| Endorsements received | ✅ Public | Count + who from |
| Accepted answer count | ✅ Public | Trust signal |
| Email address | ❌ Private | Never exposed |
| Employer name | ❌ Private | Never exposed |
| Union membership | ❌ Private | Never exposed |

**Profile is GitHub-style, not LinkedIn-style.** It shows what you've built and demonstrated — not a résumé of claims. The focus is on contributions, expertise scores, and peer-validated credentials. No follower counts. No vanity metrics.

---

### **10.4 NOS Taxonomy — Mirroring Industry Standards**

**Decision:** Robin's skill taxonomy mirrors the National Occupational Standards (NOS) published by ESDC Canada. Robin does not invent its own competency framework.
...
**How Robin uses NOS:**
- Every expertise topic maps to an NOS task code (e.g., )
- Every NOS block has a score target per role tier (APPRENTICE / JOURNEYPERSON / MASTER)
- Skill gap recommendations fire when a user's block score is below their role target
- Score targets are derived from NOS exam weighting, not invented by Robin

**Legal protection:** Robin is a mirror, not a certifier. This distinction is critical:
...
This disclaimer must appear on all skill gap recommendations, public expertise score displays, and the credential wallet. Legal review of this language is required before the skill mapping feature ships (see RESEARCH_STEPS.md — Step 6).

**NOS scope by phase:**
- Phase 1: RED_309A (Electrician Construction) only — beachhead trade
- Phase 2: Expand to RED_306A (Plumber), RED_313A (HVAC), RED_421A (Welder)
- Phase 3: Full Red Seal trade coverage
...
| **Formal mentorship matching** | Phase 2 | Q&A builds relationships organically first. Phase 2 introduces mentor tier and matching after reputation data exists to make good matches. See Section 10 Phase 2 rationale. |
| **Skill mapping / gap analysis** | Phase 2 | NOS taxonomy must be seeded first (see RESEARCH_STEPS.md). Schema and API are designed in Phase 1 but feature is behind a flag. |
| **Training resource catalogue** | Phase 2 | Depends on NOS taxonomy being seeded and validated with real tradespeople. Robin-curated first; paid placement Phase 3. |
| **Award a badge (social badges)** | Deferred indefinitely | Indistinguishable from LinkedIn endorsement glazing. Deprioritized unless user research shows distinct demand beyond professional credentials + peer endorsements. |
| **Mentor button on profiles** | Phase 2 | Not in Phase 1. Mentorship is discovered organically through Q&A patterns, not cold requests from profiles. |
| **Geographic proximity matching** | Not planned | Privacy and discrimination risk. Excluded from Robin's feature set. |
...
## **15. Pre-Build Research Requirements**

Before Phase 1 build starts, the following research work must be completed. Full detail in **RESEARCH_STEPS.md**.

| **Step** | **What** | **Blocks** |
|---|---|---|
| Step 1 | Source NOS competency blocks for Electrician 309A from ESDC | Phase 1 build |
| Step 2 | Map NOS tasks to Robin expertise topics (with tradesperson validation) | Phase 1 build |
| Step 3 | Define NOS block score targets per role tier | Skill Mapping (Phase 2) |
| Step 4 | Seed initial training resources per NOS block | Skill Mapping (Phase 2) |
| Step 5 | Validate taxonomy with 5–8 real tradespeople | Phase 1 accuracy |
| Step 6 | Legal review of skill gap and competency disclaimer language | Skill Mapping (Phase 2) |
| Step 7 | Expand NOS coverage to additional trades | Phase 2+ expansion |
| Step 8 | Training provider partnership outreach | Phase 3 revenue |

**Steps 1 and 2 are hard blockers for the Phase 1 build.**

---

## **16. Technical Implications for Related Documentation**

The decisions in this document have been reflected in the following technical files. This section summarises what changed and why.
...
**Related Documents:**
- `/docs/BUSINESS_REQUIREMENTS.md` — This document — product strategy and requirements
- `/docs/DATABASE_SCHEMA.md` — Data model
- `/docs/API_ENDPOINTS.md` — API surface
- `/docs/SYSTEM_PROMPT.md` — Code conventions
- `/docs/RESEARCH_STEPS.md` — Pre-build research tasks (NOS taxonomy, legal review, user interviews)
